...
// ../../inngest/dist/functions/processVaultDocument.js
...
function chunkText(text) {
  const chunks = [];
  const separators = ["\n\n", "\n", ". ", " "];
  function split(content, separatorIndex) {
    if (content.length <= CHUNK_SIZE)
      return [content];
    if (separatorIndex >= separators.length) {
      const result2 = [];
      for (let i = 0; i < content.length; i += CHUNK_SIZE - CHUNK_OVERLAP) {
        result2.push(content.slice(i, i + CHUNK_SIZE));
      }
      return result2;
    }
    ...
    if (current)
      result.push(current);
    return result.flatMap((chunk) => chunk.length > CHUNK_SIZE ? split(chunk, separatorIndex + 1) : [chunk]);
  }
  return split(text.trim(), 0).filter((c) => c.trim().length > 0);
}
var processVaultDocument = inngest.createFunction({
  id: "process-vault-document",
  retries: 3
}, { event: "vault/document.uploaded" }, async ({ event, step }) => {
  const { documentId, tenantId } = event.data;
  const doc = await step.run("fetch-document", async () => {
    const { data, error } = await supabase.from("vault_documents").select("*").eq("id", documentId).eq("tenant_id", tenantId).single();
    if (error || !data)
      throw new Error(`Document not found: ${documentId}`);
    return data;
  });
  const rawText = await step.run("parse-document", async () => {
    const { data, error } = await supabase.storage.from("vault-documents").download(doc.file_path);
    if (error || !data)
      throw new Error(`Failed to download: ${doc.file_path}`);
...
// ../../inngest/dist/functions/runScoutAgent.js
...
// ../../packages/agents/dist/scout/schema.js
var ScoutInputSchema = external_exports.object({
  tenantId: external_exports.string().uuid(),
  /** Organization profile / mission for matching */
  orgProfile: external_exports.object({
    name: external_exports.string(),
    mission: external_exports.string(),
    focus_areas: external_exports.array(external_exports.string()),
    annual_budget: external_exports.number().optional(),
    location: external_exports.string().optional(),
    org_type: external_exports.enum(["nonprofit", "government", "tribal", "education", "other"])
  }),
  /** Optional constraints to narrow the search */
  filters: external_exports.object({
    min_amount: external_exports.number().optional(),
    max_amount: external_exports.number().optional(),
    deadline_after: external_exports.string().optional(),
    // ISO date
    categories: external_exports.array(external_exports.string()).optional()
  }).optional()
});
var ScoutOutputSchema = external_exports.object({
  opportunities: external_exports.array(external_exports.object({
    title: external_exports.string(),
    funder: external_exports.string(),
    description: external_exports.string(),
    amount_min: external_exports.number().nullable(),
    amount_max: external_exports.number().nullable(),
    deadline: external_exports.string().nullable(),
    url: external_exports.string().url(),
    match_score: external_exports.number().min(0).max(100),
    match_rationale: external_exports.string(),
    evidence: external_exports.array(external_exports.object({
      source: external_exports.string(),
      excerpt: external_exports.string(),
      relevance: external_exports.string()
    })),
    tags: external_exports.array(external_exports.string())
  })),
  /** Total opportunities evaluated before filtering */
  total_evaluated: external_exports.number(),
  /** Model used for evaluation */
  model: external_exports.string(),
  /** Summary reasoning about the overall search */
  search_summary: external_exports.string()
});

// ../../packages/agents/dist/scout/sources/grants-gov.js
var GRANTS_GOV_API_URL = "https://www.grants.gov/grantsws/rest/opportunities/search";
async function searchGrantsGov(focusAreas) {
  try {
    ...
    return data.oppHits.map((hit) => ({
      ...
      status: "discovered",
      tags: ["federal", ...hit.cfdaList ? hit.cfdaList.map((c) => `CFDA ${c}`) : []]
    }));
  } catch (error) {
...
// ../../packages/agents/dist/scout/sources/lilly-endowment.js
import * as cheerio from "cheerio";

// ../../packages/orchestration/dist/model-router.js
function selectModel(task, plan = "free") {
  switch (task) {
    // ── Scout ──────────────────────────────────────────────────
    ...
    case "eval_judge":
      return {
        provider: "anthropic",
        model: "claude-sonnet-4-6",
        // Updated from Opus to Sonnet as requested
        costPerMillionInput: 3,
        costPerMillionOutput: 15,
        maxContextTokens: 2e5,
        supportsTools: false
...
// ../../packages/orchestration/dist/llm-client.js
import Anthropic from "@anthropic-ai/sdk";
import { GoogleGenAI } from "@google/genai";
var anthropic = new Anthropic({
  apiKey: process.env.ANTHROPIC_API_KEY
});
var google = new GoogleGenAI({
...
async function* streamLLM(request) {
  const selection = selectModel(request.task, request.plan);
  if (selection.provider !== "anthropic") {
...
// ../../packages/agents/dist/scout/sources/lilly-endowment.js
var LILLY_URL = "https://lillyendowment.org/opportunities/";
async function browseLillyEndowment() {
  try {
    const response = await fetch(LILLY_URL);
    if (!response.ok)
      throw new Error(`Lilly Endowment returned ${response.status}`);
...
// ../../inngest/dist/functions/runScoutAgent.js
var supabase2 = createClient2(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
var runScoutAgent = inngest.createFunction({
  id: "run-scout-agent",
  retries: 0
  // Avoid repeating LLM calls during dev
}, { event: "scout/run.requested" }, async ({ event, step }) => {
  const { tenantId, triggeredBy } = event.data;
  const runId = await step.run("init-run", async () => {
    const { data, error } = await supabase2.from("agent_runs").insert({
      tenant_id: tenantId,
      agent_type: "scout",
      status: "running",
  ...
  try {
    const orgProfile = await step.run("load-org-profile", async () => {
      const { data, error } = await supabase2.from("org_profiles").select("*").eq("tenant_id", tenantId).single();
      if (error || !data)
        throw new Error("Org profile not found");
      return data;
    });
...
${JSON.stringify(opp, null, 2)}`;
        ...
        if (!isEligible)
          return;
        const { data: vaultChunks } = await supabase2.from("vault_chunks").select("content, document_id").eq("tenant_id", tenantId).limit(3);
...
${JSON.stringify(opp, null, 2)}`;
        ...
        const { data: oppRecord, error: oppErr } = await supabase2.from("grant_opportunities").insert({
          tenant_id: tenantId,
          title: opp.title,
          funder: opp.funder,
        ...
        await supabase2.from("grant_matches").insert({
          tenant_id: tenantId,
    ...
    await step.run("complete-run", async () => {
      await supabase2.from("agent_runs").update({
        status: "completed",
...
// ../../inngest/dist/functions/architectStream.js
...
// ../../packages/db/dist/queries/applications.js
async function getApplication(client, id) {
  const { data } = await client.from("grant_applications").select("*").eq("id", id).is("deleted_at", null).single();
  return data;
}
async function updateSectionContent(client, applicationId, sectionId, content, tokensUsed) {
  const { data, error: fetchErr } = await client.from("grant_applications").select("outline, total_tokens_used").eq("id", applicationId).single();
  if (fetchErr || !data)
    throw new Error("Failed to fetch application for update");
  const outline = data.outline.map((s) => s.id === sectionId ? { ...s, status: "complete", content } : s);
  await client.from("grant_applications").update({
    outline,
    current_section: null,
    total_tokens_used: (data.total_tokens_used ?? 0) + tokensUsed
  }).eq("id", applicationId);
}
async function updateApplicationStatus(client, applicationId, status, extras) {
  await client.from("grant_applications").update({ status, ...extras }).eq("id", applicationId);
}

// ../../packages/db/dist/queries/librarian.js
async function hybridSearchVault(client, queryText, queryEmbedding, tenantId, options = {}) {
  ...
  if (error)
    throw new Error(`Librarian hybrid search failed: ${error.message}`);
  return (data ?? []).map((row) => ({
...
// ../../inngest/dist/functions/architectStream.js
var supabase3 = createClient4(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
var openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY });
async function broadcastToken(applicationId, token) {
  await supabase3.channel(`architect:${applicationId}`).send({
    type: "broadcast",
    event: "token",
    payload: { token }
  });
}
async function broadcastSectionComplete(applicationId, sectionId) {
  await supabase3.channel(`architect:${applicationId}`).send({
    type: "broadcast",
...
async function broadcastDraftComplete(applicationId) {
  await supabase3.channel(`architect:${applicationId}`).send({
    type: "broadcast",
...
var architectStream = inngest.createFunction({
  id: "architect-stream",
  retries: 1,
  // Timeout per section: 5 minutes
  timeouts: { finish: "5m" }
}, { event: "architect/draft.requested" }, async ({ event, step }) => {
  const { applicationId, sectionId, tenantId, plan } = event.data;
  const application = await step.run("load-application", async () => {
    return getApplication(supabase3, applicationId);
  });
  if (!application)
  ...
  await step.run("mark-drafting", async () => {
    await supabase3.from("grant_applications").update({ status: "drafting", current_section: sectionId }).eq("id", applicationId);
  });
  const librarianContext = await step.run("librarian-retrieval", async () => {
    ...
    return hybridSearchVault(supabase3, section.title, queryEmbedding, tenantId, { matchCount: 5, vectorWeight: 0.6, ftsWeight: 0.4 });
  });
  const systemPrompt = await step.run("build-system-prompt", async () => {
    const provenanceContext = librarianContext.map((r, i) => `[Source ${i + 1}: ${r.documentTitle}]
${r.content}`).join("\n\n---\n\n");
    let voiceContext = "";
    if (application.match_prior_voice) {
      const priorProposals = librarianContext.filter((r) => r.documentTitle.toLowerCase().includes("grant") || r.documentTitle.toLowerCase().includes("proposal") || r.documentTitle.toLowerCase().includes("application"));
...
${provenanceContext}${voiceContext}`;
  ...
  await step.run("persist-section", async () => {
    await updateSectionContent(supabase3, applicationId, sectionId, completedSection.text, completedSection.inputTokens + completedSection.outputTokens);
  });
  await step.run("check-completion", async () => {
    const updated = await getApplication(supabase3, applicationId);
    if (!updated)
      return;
    const allComplete = updated.outline.every((s) => s.status === "complete");
    if (allComplete) {
      await updateApplicationStatus(supabase3, applicationId, "complete");
...
// ../../inngest/dist/functions/runLiaisonAgent.js
import { createClient as createClient5 } from "@supabase/supabase-js";
var supabase4 = createClient5(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);
var runLiaisonAgent = inngest.createFunction({
  id: "run-liaison-agent",
  retries: 1
}, { event: "liaison/task.requested" }, async ({ event, step }) => {
  const { taskId, type, data, tenantId } = event.data;
  const runId = await step.run("init-run", async () => {
    const { data: run, error } = await supabase4.from("agent_runs").insert({
      tenant_id: tenantId,
      agent_type: "liaison",
      status: "running",
      model: "claude-haiku-4-5",
...
Context: ${data.context}`
      ...
      await step.run("mock-crm-sync", async () => {
        const { error } = await supabase4.from("crm_sync_logs").insert({
          tenant_id: tenantId,
